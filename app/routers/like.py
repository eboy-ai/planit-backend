...
#페이지 로드시 좋아요 수 노출
@router.get('{review_id}/like', response_model=LikeResponse)
async def get_likes(review_id:int,db:AsyncSession=Depends(get_db)):
    return await LikeService.count_likes(db,review_id)
