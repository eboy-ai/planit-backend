...
class WeatherService:
    ...
    async def get_weather(db:AsyncSession,city:str):
        ...
        # if CityWeather.
        # result = await db.execute(select(Weather).where(Weather.id == db_city.id))
        # db_weather = result.scalars().first()
        ...
        db.add(CityWeather(city_id=db_city.id, weather_id=db_weather.id))       
